import { NpmPackageType } from './npmPackageType';
...
export async function getNpmPackage(packageName) {
 ...
 * @returns
 */
export async function getAllPackageNamesFromFiles({
  folderPath = ['datasets'],
 ...
 * @param param0
 */
export async function writeNpmPackageNamesToDatabase({
  foldername = 'datasets',
  filename = 'npm-registry-namespace.json',
} = {}) {
  ...
  const packageNames = JSON.parse(data as unknown as string);
 ...
 * @returns
 */
const parseKeyName = (str: string) =>
  str.match('-') && str.length > 1 && str.match(/^[a-zA-Z]/g)
    ? camlecase(str)
    : str;

const parseKeyNames = (obj: object): object => {
...
const recursiveNpmDep =
  ([keyName1, keyName2]: [string, string]) =>
  (obj: object) =>
...
const mapFnWrapper =
  ([keyName1, keyName2]: [string, string]) =>
  ([key, value]: [string, unknown]): object => ({
    [keyName1]: key,
    [keyName2]:
      typeof value === 'object'
        ? recursiveNpmDep([keyName1, keyName2])(value)
...
export function parsePackageJson({
  ...
  if (peerDependenciesMeta) {
    returnObj.peerDependenciesMeta = Object.entries(peerDependenciesMeta).map(
      mapFnWrapper(['name', 'version'])
    );
  }
